...
# ===================================================================================
# SETUP DATASET (Versi Python Lokal)
# ===================================================================================
import zipfile

# Path ke direktori saat ini (sesuaikan dengan lokasi project Anda)
PROJECT_DIR = os.path.dirname(os.path.abspath(__file__))
DATASET_ZIP = os.path.join(PROJECT_DIR, "dataset.zip")
...
if os.path.exists(base_dir) and os.listdir(base_dir):
    print(f"Dataset sudah ada di: {base_dir}")
else:
    # Jika belum ada, ekstrak dari zip (jika file zip tersedia)
    if os.path.exists(DATASET_ZIP):
        ...
        print(f"PERINGATAN: Dataset tidak ditemukan di {base_dir}")
        print(f"Silakan letakkan folder 'dataset' atau file 'dataset.zip' di {PROJECT_DIR}")

# Tampilkan isi folder dataset
print(f"\nIsi direktori dataset ({base_dir}):")
...
# Gunakan base_dir yang sudah didefinisikan di atas
BASE_DIR = base_dir
...
# Catatan: Baris ini membutuhkan best_rf dan best_dt yang didefinisikan nanti
# Kode untuk menyimpan model dipindahkan ke bagian akhir setelah hyperparameter tuning
...
print(df_processed_preview.iloc[:, :25])  # menampilkan 25 fitur pertama agar tidak besar
...
# Pastikan xgboost sudah terinstall: pip install xgboost
...
print(df_tuned.sort_values("Accuracy", ascending=False))

"""# SHAP EXPLAINABILITY"""

# Pastikan shap sudah terinstall: pip install shap
...
shap_abs_global = np.abs(shap_values_xgb).mean(axis=(0, 2)) # Rata-rata SHAP abs per fitur, di semua sampel dan kelas
feature_order_global = np.argsort(shap_abs_global)[::-1] # Urutan fitur dari yang paling penting secara global

best_feature = feature_order_global[0] # Ambil fitur paling penting pertama (sebagai integer)

print(f"Plotting dependence for feature index: {best_feature}")
...
top_features = feature_order_global[:3]   # Use the globally ordered top features
...
print("Model berhasil disimpan: xgb_best_model.pkl, rf_best_model.pkl, dt_best_model.pkl")
...
# Assuming X and y are already loaded from a previous cell (e.g., ExgXmiTWoaJf)
# Re-split the data to ensure X_train is defined
